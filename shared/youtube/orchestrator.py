...
from .fetch import get_transcript, YouTubeManager
from youtube_transcript_api.formatters import TextFormatter
from .summarizer import summarize_text
from config.config import YOUTUBE_API_KEY
import openai
from .recipe_check import detect_recipe_from_text
from .formatter import format_summary_for_discord, format_recipe_for_discord
# import yt_dlp
import asyncio
...
async def process_youtube_video(video_id: str, summary_type: str = "default") -> dict:
    text = get_transcript(video_id)  # ✅ already joined in fetch.py
    print("[Transcript Preview]", text[:300])

    recipe = detect_recipe_from_text(text)
    if recipe:
        return {
            "type": "recipe",
            "title": recipe["title"],
            "markdown": format_recipe_for_discord(recipe),
            "raw": recipe,
    ...
    try:
        ...
        video_title = details["title"] if isinstance(details, dict) else "YouTube Video"
    except Exception:
        pass

    summary = await summarize_text(text=text, title=video_title, url=video_url, summary_type=summary_type)

    print("[YouTube Summary]", summary[:100])
    ...
    return {
        "type": "summary",
        "summary": format_summary_for_discord(summary),
        "source": video_url
    }
